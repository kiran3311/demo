...
//Generate Pending Customer Report
	...
	public void gerneratePendingCustomerReport()
		...
		test = extent
				.createTest("gerneratePendingCustomerReport",
						"Validate the pending customer report generation functionality")
				.assignAuthor(prd.getProperty("author_val")).assignCategory("Pending Cust Report");

		test.info("Generating pending customer report test start");
		...
		String startDt = "Strat Date Selected As :"
				+ prd.getProperty("CentralUserGeneratePendingCustReportStartDate_val") + " "
				+ prd.getProperty("CentralUserGeneratePendingCustReportStartMonth_val") + " "
				+ prd.getProperty("CentralUserGeneratePendingCustReportStartYear_val");
		...
		String endDt = "End Date Selected AS :" + prd.getProperty("CentralUserGeneratePendingCustReportEndDate_val")
				+ " " + prd.getProperty("CentralUserGeneratePendingCustReportEndMonth_val") + " "
				+ prd.getProperty("CentralUserGeneratePendingCustReportEndYear_val");
		...
		if (driver.switchTo().alert().getText() != null) {
			...
			test.info("Pending Customer Report : " + alertText);
			Thread.sleep(1000);
			backupScreenshot();
			test.addScreenCaptureFromPath(ss());
			Thread.sleep(500);
